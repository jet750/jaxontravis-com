...
import { HiveStore, UPGRADES } from './ui/HiveStore.js';
...
export default class PollinatorGame {
  ...
  newRun() {
    ...
    this.bee = new Bee(1600, 1850, this.progress.upgrades);
  ...
  update(dt) {
    ...
    if (this.state === 'START') {
      if (this._pendingEnter) this._beginPlay();
  ...
  _updatePlaying(dt) {
    ...
    // Rebuild spatial grid with alive enemies (used for the dash query).
    this.grid.clear();
    ...
    // Safe-pad regen (15% over 3s while stationary).
    const pad = this.meadow.isInSafePad(bee.x, bee.y);
    if (pad && bee.speed < 30) bee.regenerate(0.05, dt);
  ...
  _deposit() {
    const amount = this.bee.carriedValue;
    if (amount <= 0) return;
  ...
  _buy(upgradeId) {
    ...
    const available = this.progress.totalBanked + this.bee.carriedValue;
    ...
    if (u.kind === 'level') {
      up[u.id] = (up[u.id] || 0) + 1;
      if (u.id === 'maxHp') {
        this.bee.maxHpLevel = up.maxHp;
        this.bee.maxHp = 100 + 20 * up.maxHp;
      } else if (u.id === 'damageReduction') {
        this.bee.drLevel = up.damageReduction;
      } else if (u.id === 'attackBoost') {
        this.bee.attackLevel = up.attackBoost;
      }
    } else {
  ...
  _renderWorld(ctx) {
    ...
    // Enemies near the viewport are rendered even when frozen.
    for (const e of this.enemies) {
  ...
  _renderMinimap(ctx) {
    ...
    Minimap.draw(ctx, { bee: this.bee, meadow: this.meadow, x, y, size });
  }
}
